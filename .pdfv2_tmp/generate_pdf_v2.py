...
SRC = ROOT / 'conteudo_revisado.json'
OUT = ROOT / 'out'
OUT.mkdir(parents=True, exist_ok=True)
APPROVED_D06 = ROOT / 'approved-d06.pdf'
...
.closing{{break-before:page;padding-top:8mm}}.closing-grid{{display:grid;grid-template-columns:1fr 36mm;gap:7mm;align-items:start}}.closing-qr{{background:#103454;color:#fff;border-radius:5mm;padding:4mm;text-align:center}}.closing-qr img{{width:27mm;height:27mm;background:#fff;padding:2mm;border-radius:3mm}}.closing-qr strong{{display:block;font-size:8pt;margin-top:2mm}}.meta-table{{display:grid;grid-template-columns:35mm 1fr;margin-top:8mm;border-top:1px solid #dce5eb;font-size:7.7pt}}.meta-table dt,.meta-table dd{{padding:2mm;border-bottom:1px solid #e6edf2;margin:0}}.meta-table dt{{font-weight:700;color:#526779}}.internal-note{{margin-top:6mm;background:#fff8e8;border-left:3px solid #d49b2a;padding:3mm 4mm;font-size:7.4pt;color:#645124}}
...
def build_html(g, qr_name):
    ...
    closing=f'''<section class="closing"><div class="eyebrow">Fim do guia</div><h2>Continue na versão online</h2><div class="closing-grid"><div><p>Consulte a versão Web para aceder à versão mais recente deste guia e navegar pelos restantes conteúdos dos Guias do utilizador.</p>{resources}<p><a href="{esc(url)}">{esc(url)}</a></p></div><div class="closing-qr"><img src="{qr_name}" alt="QR code para a versão Web"><strong>Versão online</strong></div></div><dl class="meta-table"><dt>Título</dt><dd>{esc(title)}</dd><dt>Tema</dt><dd>{esc(theme)}</dd><dt>Fonte editorial</dt><dd>conteudo_revisado.json</dd><dt>Formato</dt><dd>B5, digital-first, compatível com impressão A4 ajustada</dd><dt>Data</dt><dd>22/09/2026</dd><dt>Estado</dt><dd>PDF v2 editorial</dd></dl><div class="internal-note"><strong>Nota editorial:</strong> os elementos gráficos são esquemas informativos derivados do conteúdo consolidado. Capturas reais do portal só devem ser introduzidas após confirmação do ambiente e da versão de interface a documentar.</div></section>'''
