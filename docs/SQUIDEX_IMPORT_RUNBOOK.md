...
Após cada mutação confirmada, persistir o `operationId` no journal. Em retoma, executar apenas operações pendentes do mesmo `planHash`.
...
Nunca executar rollback com `planHash` diferente do journal.
